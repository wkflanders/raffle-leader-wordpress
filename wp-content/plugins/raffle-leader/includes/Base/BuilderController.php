...
class BuilderController extends BaseController
{
    ...
    {
        if (!isset($_POST['security']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['security'])), 'raffleleader_preview_load_nonce')) {
            wp_send_json_error('Invalid security token');
            wp_die();
        }

        if (!isset($_POST['raffle_id'])) {
            wp_send_json_error('Raffle ID not provided in POST data');
            wp_die();
        }
        ...
        if ($raffle_id <= 0) {
            wp_send_json_error('Invalid Raffle ID');
            wp_die();
        }
