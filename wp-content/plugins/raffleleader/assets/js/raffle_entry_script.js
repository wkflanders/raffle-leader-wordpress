document.addEventListener("raffleLoaded", () => {
  ...
  const fontColor = emailForm.querySelector(".raffleleader-email-input").style
    .color;
  ...
  additionalEntrySections.forEach((additionalEntrySection) => {
    ...
    if (additionalEntrySection.getAttribute("data-type") === "referDetails") {
      additionalEntrySection
        ...
        .addEventListener("click", handleReferEntry);
    } else {
      additionalEntrySection
        .querySelector("button")
        .addEventListener("click", handleAdditionalEntry);
    }
  });
  ...
  async function handleEmailLogin(event) {
    ...
    try {
      ...
      totalEntries = data.data.total_entries;
      referralLink = data.data.referral_link;

      additionalEntrySections.forEach((additionalEntrySection) => {
        if (
          additionalEntrySection.getAttribute("data-type") !== "referDetails"
        ) {
          if (
            contestantEntries.some(
              (entry) =>
                entry.entry_type ===
                additionalEntrySection.getAttribute("data-type")
            )
          ) {
  ...
  function updateReferralLinkButton() {
    const referButton = document.querySelector(
      '.raffleleader-additional-entry-section[data-type="referDetails"] button'
    );
    if (referButton) {
  ...
  async function handleAdditionalEntry(event) {
    ...
    const userEntriesCounters = document.querySelectorAll(".show-user-entries");
    ...
    const match = entryType.match(
      /^[a-zA-Z]+(?=(Like|Follow|Comment|Repost)Details)/
    );
    const entryTypePrefix = match ? match[0] : null;

    if (entryTypePrefix) {
      const entryExistsWithNullDetails = contestantEntries.some((entry) => {
        const contestantEntryTypeMatch = entry.entry_type.match(
          /^[a-zA-Z]+(?=(Like|Follow|Comment|Repost)Details)/
        );
        const contestantEntryTypePrefix = contestantEntryTypeMatch
          ? contestantEntryTypeMatch[0]
          : null;

        return (
          contestantEntryTypePrefix === entryTypePrefix &&
          entry.entry_details == null
        );
      });
      ...
      if (entryExistsWithNullDetails) {
        console.log(
          `Another ${entryTypePrefix} exists but entry details are null`
        );
        inputEntryDetails = await updateEntryUIForm(entryBtn, entryTypePrefix);
      } else if (entryExistsWithNonNullDetails) {
        console.log(
          `Another ${entryTypePrefix} exists and entry details are not null`
        );
    ...
    try {
      ...
      totalEntriesCounters.forEach((totalEntriesCounter) => {
        totalEntriesCounter.querySelector("h2").innerText = totalEntries;
      });

      userEntriesCounters.forEach((userEntriesCounter) => {
        userEntriesCounter.querySelector("h2").innerText =
          contestantEntries.length;
      });
    ...
    if (isLoading) {
      ...
      setTimeout(() => {
        if (isSuccess == true) {
          ...
          additionalEntrySections.forEach((additionalEntrySection) => {
            additionalEntrySection.classList.remove(
              "inactive-additional-entry"
            );
          });
  ...
  async function updateEntryUIForm(element, entryPrefix) {
    ...
    const entryTxtCol = additionalEntrySection.querySelector(
      ".raffleleader-additional-entry-text-column"
    );
        ...
        setTimeout(() => {
          ...
          handleForm.innerHTML = `
            <form class="raffleleader-entry-handle-form">
              <input class="raffleleader-entry-handle-input" name="handle" type="username" placeholder="Enter your ${
                entryPrefix.charAt(0).toUpperCase() + entryPrefix.slice(1)
              } handle" required>
            </form>
          `;
          additionalEntrySection.appendChild(handleForm);
    
          const handleInput = handleForm.querySelector(
            ".raffleleader-entry-handle-input"
          );
          ...
          const submitForm = async (event) => {
            ...
            setTimeout(() => {
              ...
              setTimeout(() => {
                entryBtnCol.classList.remove(
                  "additional-entry-handle-form-load"
                );
                setTimeout(() => {
          ...
          handleForm.querySelector('form').addEventListener('submit', submitForm);
